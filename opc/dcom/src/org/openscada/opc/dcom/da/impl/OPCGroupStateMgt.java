...
public class OPCGroupStateMgt extends BaseCOMObject {
	public OPCGroupStateMgt(IJIComObject opcGroup)
			throws IllegalArgumentException, UnknownHostException, JIException {
		super((IJIComObject) opcGroup
				.queryInterface(Constants.IOPCGroupStateMgt_IID));
	}
	...
	public OPCGroupState getState() throws JIException {
		...
		callObject.addOutParamAsObject(new JIPointer(new JIString(
				JIFlags.FLAG_REPRESENTATION_STRING_LPWSTR)), JIFlags.FLAG_NULL);
		callObject.addOutParamAsType(Integer.class, JIFlags.FLAG_NULL);
		...
		state.setName(((JIString) ((JIPointer) result[2]).getReferent())
				.getString());
	 ...
	 * @throws JIException
	 */
	public int setState(Integer requestedUpdateRate, Boolean active,
			Integer timeBias, Float percentDeadband, Integer localeID,
			Integer clientHandle) throws JIException {
		JICallBuilder callObject = new JICallBuilder(true);
		callObject.setOpnum(1);

		callObject.addInParamAsPointer(new JIPointer(requestedUpdateRate),
				JIFlags.FLAG_NULL);
		if (active != null)
			callObject.addInParamAsPointer(new JIPointer(Integer.valueOf(active
					.booleanValue() ? 1 : 0)), JIFlags.FLAG_NULL);
		else
			callObject.addInParamAsPointer(new JIPointer(null),
					JIFlags.FLAG_NULL);
		callObject.addInParamAsPointer(new JIPointer(timeBias),
				JIFlags.FLAG_NULL);
		callObject.addInParamAsPointer(new JIPointer(percentDeadband),
				JIFlags.FLAG_NULL);
		callObject.addInParamAsPointer(new JIPointer(localeID),
				JIFlags.FLAG_NULL);
		callObject.addInParamAsPointer(new JIPointer(clientHandle),
				JIFlags.FLAG_NULL);
	...
	public OPCItemMgt getItemManagement() throws IllegalArgumentException,
	...
	public void setName(String name) throws JIException {
		...
		callObject.addInParamAsString(name,
	 ...
	 */
	public OPCGroupStateMgt clone(String name) throws JIException,
		...
		callObject.addInParamAsString(name,
				JIFlags.FLAG_REPRESENTATION_STRING_LPWSTR);
		callObject.addInParamAsUUID(Constants.IOPCGroupStateMgt_IID,
				JIFlags.FLAG_NULL);
	...
	public EventHandler attach(IOPCDataCallback callback) throws JIException {
		...
		synchronized (callbackObject) {
			String id = JIFrameworkHelper.attachEventHandler(getCOMObject(),
					Constants.IOPCDataCallback_IID, JIObjectFactory
							.buildObject(getCOMObject().getAssociatedSession(),
